...
export const ManageOrders: React.FC = () => {
  ...
  // In a real app, filter for this farmer. For now, show all.
  ...
  return (
    <div className="min-h-screen pt-32 pb-20 bg-stone-50">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <Link to="/dashboard" className="mb-6 inline-flex items-center gap-2 text-sm font-bold text-stone-500 hover:text-stone-900">
        ...
        {orders.length === 0 ? (
            ...
            <div className="space-y-6">
                {orders.map(order => (
                    <div key={order.id} className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-stone-100 transition-all hover:shadow-md">
                        ...
                        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 border-b border-stone-100 bg-stone-50/50 px-6 py-4">
                            <div className="flex items-center gap-4">
                                <span className="font-mono text-sm font-bold text-stone-500">#{order.id}</span>
                        ...
                        <div className="p-6">
                            ...
                            <div className="flex flex-wrap gap-3">
                                {order.status === 'Confirmed' && (
                                    <>
                                    <Button size="sm" onClick={() => updateOrderStatus(order.id, 'Packed')} className="rounded-full bg-stone-900 text-white">
                                        Mark as Packed
                                    </Button>
                                ...
                                {order.status === 'Packed' && (
                                    <Button size="sm" onClick={() => updateOrderStatus(order.id, 'On the way')} className="rounded-full bg-blue-600 text-white hover:bg-blue-700">
                                        <Truck className="mr-2 h-4 w-4" /> Ship Order
                                    </Button>
                                )}
                                {order.status === 'On the way' && (
                                    <Button size="sm" onClick={() => updateOrderStatus(order.id, 'Delivered')} className="rounded-full bg-green-600 text-white hover:bg-green-700">
