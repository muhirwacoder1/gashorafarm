...
interface ProductCardProps {
  product: any; // Convex product type
  onAddToCart?: (product: any) => void;
}

export const ProductCard: React.FC<ProductCardProps> = ({ product, onAddToCart }) => {
  // Fetch farmers to get farmer name
  ...
  return (
    <div className="group relative flex flex-col">
      {/* Image Container */}
      <Link to={`/product/${product._id}`} className="relative aspect-[4/5] w-full overflow-hidden rounded-[2rem] bg-white shadow-sm transition-all duration-300 group-hover:shadow-hover">
        ...
        <button
          onClick={(e) => {
            e.preventDefault();
            if (onAddToCart) {
              onAddToCart({ productId: product._id, product, quantity: 1 });
            }
          }}
      ...
      <div className="mt-4 px-2">
        <div className="flex items-start justify-between">
          ...
          <div className="text-right">
            <p className="text-xl font-bold text-stone-900">RWF {(product.price * 1300).toFixed(0)}</p>
